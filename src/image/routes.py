...
@router.get("/feed", response_model=list[ImageSchemaResponse])
async def get_feed(
        limit: int = 36,
        offset: int = 0,
...
@router.post(
    "/like", status_code=status.HTTP_201_CREATED
)
async def like_image(
    ...
    await ImageQuery.create_like(image_id, user, db)
    return {"detail": "image liked"}
